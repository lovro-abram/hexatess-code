...
* `ec_pct` MUST be a multiple of 5 between 5 and 90 (only `ec_pct/5`
  is stored).
* `data_len` is the payload byte length in UTF-8, up to 4095.
...
1. Validate `ec_pct` (multiple of 5, 5…90); encode the text to UTF-8
   (`data_len ≤ 4095`).
2. Compute `blocks = plan_blocks(data_len, ec_pct)`.
3. Compute the required bit count
   `need = 80 + 8 · (data_len + Σ ecc)` and the smallest
   `rmax ∈ 7…31` with `modules(6..rmax) ≥ need` (or honour a caller
   minimum radius).
4. Build the unmasked payload stream (§4.2).
5. Select and apply a mask (§6).
6. Build and place the mode message with the final mask id.
7. Place header + masked payload in spiral order (§3); pad the tail.
8. Stamp bullseye (§2.1) and key ring (§2.2).
